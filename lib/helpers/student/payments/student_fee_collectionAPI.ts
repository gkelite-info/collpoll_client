...
export async function fetchSemesterFeeCollections(
    collegeSemesterId: number,
) {
    const { data, error } = await supabase
        .from("student_fee_collection")
    ...
    if (error) {
        console.error(
            "fetchSemesterFeeCollections error:",
            error,
        );
        throw error;
    }
...
export async function createStudentFeeCollection(payload: {
    ...
    if (error) {
        console.error(
            "createStudentFeeCollection error:",
            error,
        );
...
export async function fetchTotalCollectedAmount(
    ...
    if (error) {
        console.error(
            "fetchTotalCollectedAmount error:",
            error,
        );
        throw error;
    }
