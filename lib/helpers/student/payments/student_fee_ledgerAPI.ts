...
export async function fetchStudentFeeLedger(studentFeeObligationId: number) {
...
export async function createStudentFeeLedger(payload: {
  ...
  if (error) {
    console.error("createStudentFeeLedger error:", error);
...
export async function fetchLedgerTotalAmount(studentFeeObligationId: number) {
  const { data, error } = await supabase
    .from("student_fee_ledger")
    .select("amount")
    .eq("studentFeeObligationId", studentFeeObligationId);
  ...
  if (error) {
    console.error("fetchLedgerTotalAmount error:", error);
    throw error;
  }
