...
export type TodayRow = {
  ...
  checkOut:     string;
  status:       string;
  classesTaken: number | null;   // Faculty only
  attendancePct: string;         // "—" if no history yet
};
...
async function getUsersForRolePaginated(
  ...
  page:       number,
  limit:      number,
): Promise<{ users: { userId: number; fullName: string }[]; totalCount: number }> {
...
export async function getHrDashCards(collegeId: number): Promise<HrDashCards> {
  ...
  if (userIds.length === 0) return { totalStaff: 0, presentToday: 0, absentToday: 0, lateCheckins: 0 };
...
export async function getMonthlyAttendance(
  ...
  if (users.length === 0) return MONTH_LABELS.map(m => ({ month: m, value: 0 }));
  ...
  return MONTH_LABELS.map((month, idx) => {
    ...
    return { month, value: Math.min(Math.round((present / (totalStaff * workingDays)) * 100), 100) };
  });
}
...
export async function getTodayAttendance(
  ...
  const today = todayDate();
  const { users, totalCount } = await getUsersForRolePaginated(collegeId, role, page, limit);
  ...
  const { data: dailyRows } = await supabase
    .from("attendance_daily")
    .select("attendanceDailyId, userId, checkIn, checkOut, status, classesTaken")
    .in("userId", userIds)
    .eq("attendanceDate", today);
  ...
  // Fetch adjustments
  const adjMap = new Map<number, { newCheckIn: string|null; newCheckOut: string|null }>();
  if (dailyIds.length > 0) {
    ...
    for (const a of adjs ?? []) {
      if (!adjMap.has(a.attendanceDailyId))
        adjMap.set(a.attendanceDailyId, { newCheckIn: a.newCheckIn, newCheckOut: a.newCheckOut });
    }
  }
  ...
  const workingDays = new Set((monthRows ?? []).map(r => r.attendanceDate)).size || 1;
  const pctMap = new Map<number, number>();
  for (const uid of userIds) {
    const userRows = (monthRows ?? []).filter(r => r.userId === uid);
    const present  = userRows.filter(r => r.status === "PRESENT" || r.status === "LATE").length;
...
export async function getMonthDetail(
  ...
  const workingDays = new Set((rows ?? []).map(r => r.attendanceDate)).size || 1;
  ...
  const data = users.map(u => {
    ...
    const pct         = Math.min(Math.round(((presentDays + lateDays) / workingDays) * 100), 100);
    ...
    return {
      ...
      performance:   pct >= 90 ? "Excellent" : pct >= 75 ? "Good" : "Needs Improvement",
    };
  });
