...
export async function shareCampusBuzzPost(
    payload: {
        campusBuzzPostId: number;
        sharedTo: string; // ex: "whatsapp", "email", "copy-link"
    },
    userId: number,
) {
...
export async function fetchCampusBuzzPostShareCount(
    campusBuzzPostId: number,
) {
...
export async function fetchCampusBuzzPostShares(
    campusBuzzPostId: number,
) {
    const { data, error } = await supabase
        .from("campus_buzz_post_shares")
...
export async function fetchCampusBuzzPostSharesByUser(
    userId: number,
) {
    const { data, error } = await supabase
        .from("campus_buzz_post_shares")
