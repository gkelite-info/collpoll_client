...
type SubjectUpsertPayload = SubjectDBPayload & {
    createdAt?: string;
    updatedAt: string;
};

export type SubjectDBPayload = {
    collegeSubjectId?: number; // 🔹 for edit
    subjectName: string;
    subjectCode: string;
...
export async function resolveSubjectUIFromIds({
    collegeEducationId,
    collegeBranchId,
    ...
    const [{ data: edu }, { data: br }, { data: yr }, { data: sem }] =
        await Promise.all([
            supabase.from("college_education")
                .select("collegeEducationType")
                .eq("collegeEducationId", collegeEducationId)
                .single(),

            supabase.from("college_branch")
...
export async function getAcademicSubjects(collegeId: number) {
    const { data, error } = await supabase
      ...
      subjectKey,
      credits,
      collegeEducation:college_education (
        collegeEducationType
      ),
      collegeBranch:college_branch (
        collegeBranchCode
      ),
      collegeAcademicYear:college_academic_year (
        collegeAcademicYear
      ),
      collegeSemester:college_semester (
        collegeSemester
      )
    `)
        .eq("collegeId", collegeId)
...
export const getAcademicSubjectById = async (
    ...
    try {
        ...
        console.error("getAcademicSubjectById error:", err);
        return {
            success: false,
            error: err.message || "Failed to fetch subject",
        };
    }
};

export const upsertAcademicSubject = async (
    payload: SubjectDBPayload
) => {
    try {
        const now = new Date().toISOString();

        const isEdit = !!payload.collegeSubjectId;

        const upsertPayload: SubjectUpsertPayload = {
            ...payload,
            subjectCode: payload.subjectCode.toUpperCase(),
            subjectKey: payload.subjectKey
                ? payload.subjectKey.toUpperCase()
                : null,
            ...
            // createdAt: payload.collegeSubjectId ? undefined : now,
        };
...
export const softDeleteAcademicSubject = async (
    collegeSubjectId: number
) => {
    try {
        ...
        console.error("softDeleteAcademicSubject error:", err);
        return {
            success: false,
