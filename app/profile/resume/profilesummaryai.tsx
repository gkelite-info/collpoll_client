...
import {
  fetchAllResumeData,
  ResumeData,
} from "@/lib/helpers/student/Resume/Resumedatafetcher";
import { calculateATSScore } from "@/lib/helpers/student/Resume/atsScoreCalculator";
...
function buildProfileParagraphFromResumeData(data: ResumeData): string {
  ...
  if (data.education.length > 0) {
    const eduLevels = data.education
      .map((e) => {
        if (e.educationLevel === "primary")
          return `schooling at ${e.institutionName}`;
        if (e.educationLevel === "secondary")
          return `intermediate at ${e.institutionName}`;
        if (e.educationLevel === "undergraduate") {
          return `${e.courseName || "UG"} from ${e.institutionName}${e.cgpa ? ` (CGPA: ${e.cgpa})` : e.percentage ? ` (${e.percentage}%)` : ""}`;
        }
        if (e.educationLevel === "masters") {
  ...
  if (data.internships.length > 0) {
    const internshipList = data.internships
      .map(
        (internship) =>
          `${internship.role} at ${internship.organizationName.trim()}${internship.domain ? ` (${internship.domain})` : ""}`,
      )
      .join("; ");
    parts.push(`Internship experience includes ${internshipList}.`);
  }
  ...
  if (data.projects.length > 0) {
    const projectList = data.projects
      .map(
        (project) =>
          `${project.projectName}${project.toolsAndTechnologies?.length ? ` (${project.toolsAndTechnologies.join(", ")})` : ""}`,
      )
      .join("; ");
    parts.push(`Projects include ${projectList}.`);
  }
  ...
  if (data.clubs.length > 0) {
    parts.push(
      `Active in ${data.clubs.map((club) => `${club.clubName} as ${club.role}`).join(", ")}.`,
    );
  }

  if (data.exams.length > 0) {
    parts.push(
      `Competitive exams: ${data.exams.map((exam) => `${exam.examName} - ${exam.score}`).join(", ")}.`,
    );
  }
...
function buildProfileParagraphFromAllData(data: any): string {
    ...
    const eduLevels = data.education
      ...
      .map((e: any) => {
        ...
        if (e.educationLevel === "masters")
          return `${e.courseName || "Masters"} from ${e.institutionName}${e.cgpa ? ` (CGPA: ${e.cgpa})` : ""}`;
        if (e.educationLevel === "phd")
          return `PhD from ${e.institutionName}${e.researchArea ? ` in ${e.researchArea}` : ""}`;
        return null;
      })
      .filter(Boolean);
    if (eduLevels.length > 0) {
  ...
  if (data?.skills?.technical?.length > 0) {
    parts.push(
      `Technical skills include ${data.skills.technical.map((s: any) => s.name).join(", ")}.`,
    );
  }

  if (data?.skills?.tools?.length > 0) {
    parts.push(
      `Tools and frameworks expertise covers ${data.skills.tools.map((s: any) => s.name).join(", ")}.`,
    );
  }

  if (data?.skills?.soft?.length > 0) {
    parts.push(
      `Soft skills encompass ${data.skills.soft.map((s: any) => s.name).join(", ")}.`,
    );
  }
  ...
  if (data?.internships?.length > 0) {
    ...
    if (active.length > 0) {
      const list = active
        .map(
          (i: any) =>
            `${i.role} at ${i.organizationName.trim()}${i.domain ? ` (${i.domain})` : ""}`,
        )
        .join("; ");
      parts.push(`Internship experience includes ${list}.`);
    }
  ...
  if (data?.projects?.length > 0) {
    ...
    if (active.length > 0) {
      const list = active
        .map(
          (p: any) =>
            `${p.projectName}${p.toolsAndTechnologies?.length > 0 ? ` (${p.toolsAndTechnologies.join(", ")})` : ""}`,
        )
        .join("; ");
      parts.push(`Projects include ${list}.`);
    }
  ...
  if (data?.certifications?.length > 0) {
    ...
    if (active.length > 0) {
      parts.push(
        `Certified in ${active.map((c: any) => c.certificationName).join(", ")}.`,
      );
    }
  }
  ...
  if (data?.awards?.length > 0) {
    ...
    if (active.length > 0) {
      parts.push(
        `Accomplishments include ${active.map((a: any) => `${a.awardName} (by ${a.issuedBy.trim()})`).join(", ")}.`,
      );
    }
  }
  ...
  if (data?.clubs?.length > 0) {
    ...
    if (active.length > 0) {
      parts.push(
        `Active in ${active.map((c: any) => `${c.clubName} as ${c.role}`).join(", ")}.`,
      );
    }
  }
  ...
  if (data?.competitiveExams?.length > 0) {
    ...
    if (active.length > 0) {
      parts.push(
        `Competitive exams: ${active.map((e: any) => `${e.examName} — ${e.score}`).join(", ")}.`,
      );
    }
  }
  ...
  if (data?.academicAchievements?.length > 0) {
    ...
    if (active.length > 0) {
      parts.push(
        `Academic achievements include ${active.map((a: any) => a.achievementName).join(", ")}.`,
      );
    }
  }
...
function JDSkillGapBox({
  ...
  return (
    <div className="mt-5 rounded-xl border border-purple-100 bg-gradient-to-br from-[#faf7ff] to-[#f3eeff] p-4 md:p-5">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-7 h-7 rounded-full bg-[#7c3aed] flex items-center justify-center flex-shrink-0">
          <svg
        ...
        <div>
          ...
          <p className="text-[11px] text-purple-500 font-medium">
            See how your skills match and what to add
          </p>
        </div>
      </div>
      ...
      {isGenerating ? (
        ...
        <>
          {matchingSkills.length > 0 && (
            <div className="mb-4 bg-white rounded-xl p-4 border border-green-100">
              <p className="text-xs font-bold text-green-600 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                <span className="w-4 h-4 rounded-full bg-green-500 flex items-center justify-center text-white text-[9px] shrink-0">
                  ✓
                </span>
                Your Skills — Already Strong for This JD
              </p>
              <div className="flex flex-wrap gap-2">
                {matchingSkills.map((skill, i) => (
                  <span
                    key={i}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1 cursor-default ${skill.demand === "high" ? "bg-green-50 text-green-700 border-2 border-green-400" : "bg-green-50 text-green-600 border border-green-200"}`}
                  >
                    ✓ {skill.name}
                    {skill.demand === "high" && (
          ...
          {suggestedSkills.length > 0 && (
            <div className="mb-4 bg-white rounded-xl p-4 border border-blue-100">
              ...
              <p className="text-[11px] text-gray-400 mb-3">
                These are in your skill master but not yet on your resume.
                Select to add them.
              </p>
              <div className="flex flex-wrap gap-2">
                {suggestedSkills.filter((s) => s.demand === "high").length >
                  0 && (
                  <>
                ...
                {suggestedSkills.filter((s) => s.demand === "medium").length >
                  0 && (
                  <>
          ...
          {missingSkills.length > 0 && (
            <div className="mb-4 bg-white rounded-xl p-4 border border-orange-100">
              ...
              <p className="text-[11px] text-gray-400 mb-3">
                The JD demands these. Consider learning them to strengthen your
                profile.
              </p>
              <div className="flex flex-wrap gap-2">
                {missingSkills.map((skill, i) => {
          ...
          <p className="text-[11px] text-purple-600 font-semibold mt-2 bg-purple-50 rounded-lg px-3 py-2 border border-purple-100">
            💼 Select skills from the blue section above and hit{" "}
            <strong>Save Skills</strong> to update your resume.
          </p>
        </>
      )}
    </div>
...
function ProfileSummaryBox({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 md:px-5 py-3 md:py-3.5 border-b border-gray-100 bg-gray-50 gap-3 sm:gap-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {atsLoading && (
            <div className="flex items-center gap-1.5 text-xs text-gray-400 font-medium w-full sm:w-auto mt-1 sm:mt-0">
              <svg
          ...
          {atsScore !== null && !atsLoading && (
            <div
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border-2 w-full sm:w-auto mt-1 sm:mt-0 ${
                atsScore >= 75
        ...
        <button
          onClick={onGenerate}
          disabled={isGenerating}
          className="w-full sm:w-auto justify-center text-[#282828] text-sm font-semibold px-6 py-2.5 rounded-lg shadow bg-[linear-gradient(to_right,#EDE1FE_5%,#D5BFFF_95%)] disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer flex items-center gap-2 hover:opacity-90 transition-opacity"
        >
          {isGenerating ? (
            <>
              <svg
      ...
      <div className="px-4 md:px-5 py-4">
        {profileDataLoading ? (
          ...
          <textarea
            ...
            placeholder="Your profile data from 16 tables will appear here…"
            rows={6}
            className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 md:px-4 py-3 outline-none text-sm text-gray-600 resize-none placeholder-gray-400 focus:border-[#16284F] transition-colors"
          />
        )}
        <div className="flex justify-end mt-3">
          <button
            onClick={onSave}
            disabled={isSaving || !editableSummary.trim() || profileDataLoading}
            className="w-full sm:w-auto justify-center text-white text-sm font-semibold px-6 py-2.5 rounded-lg bg-[#2ecc8a] hover:bg-[#27b87a] disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer flex items-center gap-2 transition-all"
          >
            {isSaving && selectedSummaryIdx === -1 ? (
              <>
                <svg
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 md:px-5 py-3 md:py-3.5 border-b border-gray-100 bg-gray-50 gap-3 sm:gap-0">
        <div className="flex items-center gap-2">
          <p className="text-xs font-bold text-[#16284F] uppercase tracking-widest">
            JD-Based Summary Suggestions
          </p>
          <span className="text-[10px] bg-[#EEEDFE] text-[#3C3489] px-2 py-0.5 rounded-full font-semibold border border-purple-200">
            AI Powered
          </span>
        </div>
      </div>
      ...
      {aiSummaries.length > 0 && (
        <div className="px-4 md:px-5 pb-4 md:pb-5 flex flex-col gap-3 border-t border-gray-100 pt-4">
          <div className="flex items-start md:items-center gap-2 bg-[#EDE1FE] border border-[#c4a8fc] rounded-xl px-4 py-3">
            <svg
              className="w-4 h-4 text-[#6b5cff] flex-shrink-0 mt-0.5 md:mt-0"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
            ...
            <p className="text-xs text-[#4c3d9e] font-semibold leading-relaxed md:leading-normal">
              <span className="font-black text-[#16284F]">
                5 AI-Suggested Summaries — Select the one that best fits you.
              </span>
            </p>
          </div>

          {atsScore !== null && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-wrap bg-gradient-to-r from-[#faf7ff] to-[#f0fdf7] border border-purple-100 rounded-xl px-4 py-3">
              <div className="flex items-center gap-2">
                <span className="text-[11px] text-gray-500 font-semibold">
                  Before AI
                </span>
                <span
                  className={`text-sm font-black px-2.5 py-1 rounded-full ${atsScore >= 75 ? "bg-green-100 text-green-700" : atsScore >= 50 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-600"}`}
                >
                  {atsScore}/100
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-black px-2.5 py-1 rounded-full bg-purple-100 text-purple-700">
                  {selectedSummaryIdx !== null && selectedSummaryIdx >= 0
                    ? "Score updated ✓"
                    : "Select a summary ↓"}
                </span>
              </div>
              <span className="sm:ml-auto text-[10px] text-gray-400 mt-1 sm:mt-0">
                Pick one below to replace your current summary
              </span>
            </div>
          )}
          ...
          {aiSummaries.map((summary, idx) => (
            <div
              key={idx}
              className={`rounded-xl p-4 md:p-5 border-2 transition-all ${selectedSummaryIdx === idx ? "border-[#2ecc8a] bg-[#f0fdf7]" : "border-gray-100 bg-white shadow-sm"}`}
            >
              <div className="flex flex-col md:flex-row items-start justify-between gap-3">
                ...
                <button
                  onClick={() => onUseSummary(summary, idx)}
                  disabled={isSavingSummary}
                  className={`w-full md:w-auto shrink-0 text-white text-xs font-semibold px-4 py-2.5 rounded-lg transition-all cursor-pointer disabled:cursor-not-allowed ${selectedSummaryIdx === idx ? "bg-[#16a85a]" : "bg-[#2ecc8a] hover:bg-[#27b87a]"} disabled:opacity-60 justify-center flex`}
                >
                  {isSavingSummary && selectedSummaryIdx === idx ? (
                    <span className="flex items-center gap-1.5">
                      <svg
...
export default function ProfileSummaryCard() {
  ...
  const [isGenerating, setIsGenerating] = useState(false);
  const [aiSummaries, setAiSummaries] = useState<string[]>([]);
  const [selectedSummaryIdx, setSelectedSummaryIdx] = useState<number | null>(
    null,
  );
  const [isSavingSummary, setIsSavingSummary] = useState(false);
  ...
  const [jdMatchingSkills, setJdMatchingSkills] = useState<
    Array<{ name: string; demand: "high" | "medium" }>
  >([]);
  ...
  const [jdMatchingSkillsTab, setJdMatchingSkillsTab] = useState<
    Array<{ name: string; demand: "high" | "medium" }>
  >([]);
  const [jdSuggestedSkillsTab, setJdSuggestedSkillsTab] = useState<
    Array<{ name: string; resumeSkillId: number; demand: "high" | "medium" }>
  >([]);
  const [jdMissingSkillsTab, setJdMissingSkillsTab] = useState<
    Array<{ name: string; demand: "high" | "medium" }>
  >([]);
  const [isGeneratingGapTab, setIsGeneratingGapTab] = useState(false);
  const [selectedMissingSkillsTab, setSelectedMissingSkillsTab] = useState<
    Set<string>
  >(new Set());
  ...
  useEffect(() => {
    ...
    const nextSkillsText = jobDescription.trim()
      ? jobDescription
      : editableSummary;
    setSkillsJD(nextSkillsText);
  }, [jobDescription, editableSummary, hasEditedSkillsJD]);
  ...
  const processSkillGap = async (
    jd: string,
    setMatching: (
      v: Array<{ name: string; demand: "high" | "medium" }>,
    ) => void,
    setSuggested: (
      v: Array<{
        name: string;
    ...
    try {
      ...
      const { matching, missing } = await suggestSkillsFromJDWithDemand(
        jd,
        allSkillNames,
      );
      const nameToSkill = new Map(
      ...
      if (matchingOnResume.length > 0 || toAdd.length > 0) {
        toast.success(
          `Skill analysis complete! ${toAdd.length} skills to add, ${matchingOnResume.length} already strong.`,
        );
      } else if (missing.length > 0) {
        toast.success(
          `Analysis complete! ${missing.length} skills to learn for this JD.`,
        );
      } else {
        toast.success("You already have all skills for this JD! 🎉");
      }
  ...
  const handleManualSave = async () => {
    ...
    try {
      ...
      if (isUpdating) {
        ...
        const result = await upsertProfileSummary(
          studentId,
  ...
  const handleSuggestSkillsFromJD = async () => {
    const sourceText =
      skillsJD.trim() || jobDescription.trim() || editableSummary.trim();
  ...
  return (
    <div className="p-4 md:p-6 bg-[#FFFFFF] min-h-screen rounded-xl mx-2 md:mx-0">
      <button
        onClick={() => router.push("/profile?resume=profile-summary&Step=10")}
        className="flex items-center justify-center w-8 h-8 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors mb-4"
      >
      ...
      <div className="flex gap-2 md:gap-3 mb-4 md:mb-6 overflow-x-auto scrollbar-hide pb-1">
        <button
          onClick={() => setActiveTab("personal")}
          className={`whitespace-nowrap px-4 md:px-6 py-2 rounded-lg text-sm font-semibold transition cursor-pointer ${activeTab === "personal" ? "bg-[#16284F] text-white" : "bg-[#eef1f7] text-gray-600 hover:bg-[#e2e6f0]"}`}
        >
          Personal Details
        </button>
        <button
          onClick={() => setActiveTab("keyskills")}
          className={`whitespace-nowrap px-4 md:px-6 py-2 rounded-lg text-sm font-semibold transition cursor-pointer ${activeTab === "keyskills" ? "bg-[#16284F] text-white" : "bg-[#eef1f7] text-gray-600 hover:bg-[#e2e6f0]"}`}
        >
          Key Skills
        </button>
      </div>
      ...
      {activeTab === "personal" && (
        <div className="bg-white rounded-2xl p-4 md:p-6 max-w-4xl">
          <div className="mb-4 md:mb-5">
            <div
              style={{
                background: "linear-gradient(90deg, #F1E7FE 0%, #D4BEFF 100%)",
              }}
              className="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-[#6b5cff]"
            >
              <Image src="/AI Robot.png" alt="AI" width={40} height={40} />
      ...
      {activeTab === "keyskills" && (
        <div className="bg-white rounded-2xl p-4 md:p-6 max-w-4xl">
          ...
          <div className="bg-white rounded-xl p-4 md:p-5 mb-6 shadow-sm border border-gray-100">
            ...
            />
            <p className="text-[11px] text-gray-400 mt-2">
              JD text from Personal Details auto-fills here. If no JD is added,
              your current professional summary is used.
            </p>
            <div className="flex justify-end mt-4">
              <button
            ...
            <JDSkillGapBox
              ...
              onToggleMissing={toggleMissingSkillTab}
            />
            {jdSuggestedSkillsTab.some((s) =>
              selectedSkillIds.has(s.resumeSkillId),
            ) && (
              <div className="flex justify-end mt-3">
                <button
                  onClick={handleSaveSkills}
                  disabled={isSavingSkills}
                  className="w-full sm:w-auto justify-center bg-[#16284F] text-white text-sm font-semibold px-8 py-2.5 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer hover:bg-[#1d3566] transition-all flex items-center gap-2"
                >
                  {isSavingSkills ? (
                    <>
                      <svg
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mt-8 pt-4 border-t border-gray-100 gap-4 sm:gap-0">
            <span className="text-sm text-gray-500">
      ...
      <div className="flex justify-center mt-10">
        <button
          ...
          disabled={isNavigating}
          className="w-full sm:w-auto justify-center bg-[#16284F] text-white text-sm font-semibold px-12 py-3 rounded-xl shadow-sm flex items-center gap-2 cursor-pointer disabled:cursor-not-allowed hover:bg-[#1d3566] transition-all"
        >
          {isNavigating ? (
            <>
              <svg
