...
import {
  calculateATSScore,
  ATSResult,
} from "@/lib/helpers/student/Resume/atsScoreCalculator";
...
export default function ProfileSummary() {
  ...
  const waitForToast = () => new Promise((resolve) => setTimeout(resolve, 700));
  ...
  const saveSummary = async (): Promise<boolean> => {
    ...
    if (trimmed.length < 20) {
      toast.error(
        "Summary is too short. Please write at least a few sentences.",
      );
      return false;
    }
    const isUpdating = Boolean(resumeSummaryId);
  ...
  const handleSubmit = async () => {
    ...
    if (suggestions.length > 0 && !selectedVersion) {
      toast.error(
        "Please select one of the AI-generated summaries, or edit the text above.",
      );
      return;
    }
  ...
  return (
    <div className="min-h-[58vh] bg-gray-100 flex justify-center rounded-xl mt-2 mb-5 mx-2 md:mx-0">
      <div className="w-full bg-white rounded-xl shadow-sm p-4 md:p-6">
        <div className="flex items-center justify-between mb-4 md:mb-6">
          <h2 className="text-lg md:text-xl font-medium text-[#282828]">
            Profile Summary
          </h2>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-4 mb-3 p-3.5 md:p-4 rounded-xl bg-gradient-to-r from-violet-500 via-purple-500 to-indigo-500 shadow-sm">
            <div className="flex items-start gap-2.5 flex-1 min-w-0">
              <span className="text-white text-base mt-0.5 shrink-0">💡</span>
              <p className="text-xs text-white/90 leading-relaxed">
                Type or paste your existing profile summary below, then click{" "}
                <span className="font-bold text-white">"Generate with AI"</span>{" "}
                to get{" "}
                <span className="font-bold text-white">
                  5 improved versions
                </span>{" "}
                — each with a different tone and focus. Pick the one that best
                represents you!
              </p>
            </div>
            <button
              onClick={handleGenerateAI}
              disabled={isGenerating}
              className={`flex items-center justify-center w-full md:w-auto gap-2 px-4 py-2.5 md:py-2 text-sm rounded-lg font-semibold transition-all shrink-0 ${
                isGenerating
                  ? "bg-white/20 text-white/50 cursor-not-allowed"
                  : "bg-white text-violet-700 cursor-pointer hover:bg-violet-50 shadow-md"
              }`}
            >
              {isGenerating ? (
                ...
                <>
                  <span className="text-violet-500 text-base leading-none">
                    ✦
                  </span>
                  Generate with AI
                </>
          ...
          {hasSuggestions && (
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <p className="text-xs text-amber-500">
                Clicking a version will replace current text
              </p>
            </div>
          )}
          ...
          <div className="relative">
            <textarea
              ...
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Paste or type your current profile summary here... e.g. A passionate Computer Science student with a strong interest in software development..."
              className="w-full border border-[#CCCCCC] rounded-lg p-3 md:p-4 text-sm text-[#525252] focus:outline-none resize-none focus:border-violet-400 transition-colors"
            />
            {selectedVersion && (
              <div className="absolute top-3 right-3">
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-violet-100 text-violet-600 font-medium border border-violet-200">
                  {VERSION_STYLES[selectedVersion]?.icon}{" "}
                  {
                    suggestions.find((s) => s.version === selectedVersion)
                      ?.label
                  }
                </span>
              </div>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    <svg width="72" height="72" viewBox="0 0 72 72">
                      ...
                      <circle
                        ...
                        strokeDashoffset={
                          2 * Math.PI * 28 -
                          (beforeScore.total / 100) * 2 * Math.PI * 28
                        }
                        transform="rotate(-90 36 36)"
                        style={{ transition: "stroke-dashoffset 1s ease" }}
                      />
                  ...
                  <div className="flex-1">
                    <span
                      className="inline-block text-[10px] font-bold px-2 py-0.5 rounded-full mb-2"
                      style={{
                        background: `${beforeScore.color}20`,
                        color: beforeScore.color,
                      }}
                    >
                      {beforeScore.label}
                    </span>
            ...
            <div className="rounded-xl bg-gradient-to-br from-violet-100 via-purple-50 to-indigo-100 border border-purple-100 p-4 flex flex-col items-center justify-between text-center">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 w-full mb-3 text-center sm:text-left">
                <img
                  src="/AI Robot.png"
                  alt="AI Robot"
                  width={64}
                ...
                <p className="flex-1 text-sm font-medium text-gray-700 leading-snug">
                  Do you feel your current resume doesn't fully showcase your
                  skills or match the job you're aiming for?
                  <br />
          ...
          {!isGenerating && hasSuggestions && (
            <div className="mt-5">
              ...
              <div className="mb-3 p-2.5 rounded-lg bg-amber-50 border border-amber-100 flex items-start gap-2">
                <span className="text-amber-500 shrink-0 text-sm">⚠️</span>
                <p className="text-[11px] text-amber-700 leading-relaxed">
                  Each version below uses a{" "}
                  <span className="font-semibold">different writing style</span>{" "}
                  — from concise &amp; punchy to achievement-focused to
                  leadership-oriented. Click{" "}
                  <span className="font-semibold">"Use This"</span> to apply it
                  to your summary above. Your current text will be{" "}
              ...
              <div className="space-y-3">
                {suggestions.map((s, i) => {
                  const style =
                    VERSION_STYLES[s.version] ?? VERSION_STYLES["v1"];
                  const isSelected = selectedVersion === s.version;
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-2.5 gap-3 sm:gap-0">
                        ...
                        <div className="flex items-center w-full sm:w-auto">
                          <button
                            ...
                            onClick={() => handleUseSuggestion(s)}
                            disabled={isSelected}
                            className={`w-full sm:w-auto text-[11px] px-3 py-2 sm:py-1.5 rounded-md font-medium transition-all ${
                              isSelected
                                ? `${style.bg} ${style.accent} border-2 ${style.border} cursor-default`
                                : `bg-gray-900 text-white hover:bg-gray-700 cursor-pointer`
                            }`}
          ...
          <div className="flex justify-end mt-4">
            <button
              onClick={handleSubmit}
              disabled={isSubmitting}
              className={`w-full sm:w-auto px-8 py-2.5 sm:py-2 rounded-md text-sm font-medium text-white min-w-[110px] ${
                isSubmitting
                  ? "bg-gray-400 cursor-not-allowed"
                  : "bg-[#43C17A] cursor-pointer hover:bg-[#3bad6d]"
              }`}
