export default function MeetingsShimmer({ 
    hideToolbar = false,
    gridHoursLength = 11, // 8 AM to 6 PM default
    daysLength = 7
}: { 
    hideToolbar?: boolean;
    gridHoursLength?: number;
    daysLength?: number;
}) {

    const gridHours = Array.from({ length: gridHoursLength }, (_, i) => i + 8); // Starts at 8 AM
    const shimmerDays = Array.from({ length: daysLength }, (_, i) => i);

    return (
        <>
        <div className={`w-full flex flex-col animate-pulse overflow-hidden relative shrink-0 ${hideToolbar ? 'flex-1 h-full' : 'h-[calc(100vh-40px)] min-h-[850px] bg-white rounded-2xl shadow-sm border border-gray-200 mb-5'}`}>
            ...
            <div className="flex-1 overflow-y-auto overflow-x-auto custom-scrollbar relative bg-white">
                <div className="min-w-[800px] flex flex-col relative w-full">
                    ...
                    <div className="flex border-b border-gray-200 sticky top-0 bg-white z-30 shadow-sm pr-4">
                        ...
                        <div className="flex-1 grid" style={{ gridTemplateColumns: `repeat(${daysLength}, minmax(0, 1fr))` }}>
                            {shimmerDays.map(day => (
                    ...
                    <div className="flex flex-1 relative bg-white pr-4 pb-4">
                        ...
                        <div className="flex-1 grid relative overflow-hidden" style={{ gridTemplateColumns: `repeat(${daysLength}, minmax(0, 1fr))` }}>
        ...
        {!hideToolbar && <div className="h-8 shrink-0 w-full"></div>}
        </>
    );
}
