...
import { getExpenseAttachmentProxyUrl, type EmployeeExpenseReport } from "@/lib/helpers/reimbursements/employeeExpenseReportsAPI";
...
export default function ReimbursementDetailsModal({ report, onClose }: Props) {
  const openAttachment = (filePath: string, download = false) => {
    try {
      const url = getExpenseAttachmentProxyUrl(filePath);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/55 px-4 py-6 lg:pl-[calc(17vw+1rem)]">
      <div className="relative flex max-h-[92vh] w-full max-w-[900px] flex-col overflow-hidden rounded-[16px] bg-[#F8FAFC] shadow-2xl [&_button]:cursor-pointer">
        <div className="relative z-10 flex-none bg-[#F8FAFC] px-6 pb-4 pt-6 shadow-[0_4px_6px_-4px_rgba(0,0,0,0.05)]">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4 pr-9">
            <div><h2 className="text-[24px] font-bold text-[#14213A]">{report.expenseTitle}</h2><p className="mt-1 text-[13px] text-[#61708A]">Submitted on {submitted}</p></div>
