...
export default function AddAnnouncementModal({
  ...
  const availableRoles = (role ? roleOptionsMap[role] || [] : []).filter(r => isSchool ? (r !== "PlacementOfficer" && r !== "Parent") : true);
  ...
  const handleSave = async () => {
    ...
    if (date < today) {
      toast.error(t("Past dates are not allowed"));
      return;
    }

    if (!userId || !collegeId || !role) {
      toast.error(t("User session not loaded"));
      return;
    }
    ...
    try {
      ...
      for (const roleItem of targetRoles) {
        ...
        if (!roleRes.success) {
          toast.error(t("Failed to assign announcement role"));
          return;
        }
      }
