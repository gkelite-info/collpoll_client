...
//     const emailPayloads = uniqueUsers.map((u) => ({
...
//     return NextResponse.json({ success: true, count: uniqueUsers.length });
//   } catch (error: any) {
...
export async function POST(req: Request) {
  try {
    ...
    // ==========================================
    // --- GATEKEEPER: CHECK USER PREFERENCES ---
    // ==========================================
    ...
    const optedInUsers = uniqueUsers.filter((user) => {
      // Always allow manual external emails (userId 0)
      if (user.userId === 0) return true;

      const userPref = preferences.find((p) => p.userId === user.userId);
      // If preference exists and is explicitly false, block it. Otherwise, allow.
