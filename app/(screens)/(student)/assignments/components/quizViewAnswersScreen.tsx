...
function QuizViewAnswersScreenContent({ quiz }: { quiz: any }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f4f4f4] rounded-lg -m-2 p-2 md:p-4 relative">
      <div className="flex justify-between items-start mb-2 px-2 pt-2 md:px-0 md:pt-0">
        <div className="flex gap-1">
          <CaretLeftIcon
            size={24}
            className="text-[#282828] cursor-pointer active:scale-90 mt-0.5"
            weight="bold"
            onClick={handleBack}
          />
          <div className="flex flex-col min-w-0">
            <h2 className="text-lg md:text-xl font-bold text-[#282828] truncate">
              {quiz?.courseName || "Quiz"}
            </h2>
            <p className="text-xs md:text-sm font-medium text-[#282828] truncate">
              {quiz?.topic || ""}
            </p>
          </div>
        </div>
        <div className="text-sm md:text-xl font-bold text-[#282828] mt-1 md:mt-8 shrink-0">
          {t("Score :")}{" "}
          <span className="text-[#43C17A]">{quiz?.score || "-"}</span>
        </div>
      </div>

      <div className="mb-4 md:mb-6 px-2 md:px-0 mt-2 md:mt-0">
        <div className="h-2 w-full md:h-2.5 bg-[#dcfce7] rounded-full overflow-hidden">
          <div
      ...
      <div className="flex-1 overflow-y-auto max-h-[75vh] md:max-h-[70vh] pb-5 space-y-3 md:space-y-4 px-1 md:px-0">
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <span
                  className={`px-4 py-1 text-sm font-bold rounded-md text-white ${isCorrect ? "bg-[#43C17A]" : "bg-[#FF3B30]"}`}
                >
              ...
              {q.questionType === "Multiple Choice" ? (
                <div className="flex flex-col gap-2">
                  {q.quiz_question_options
                    ...
                    .map((opt: any) => {
                      const isSelected =
                        studentAnswer?.selectedOptionId === opt.optionId;
                      ...
                      return (
                        <div
                          key={opt.optionId}
                          className={`flex items-center gap-3 px-3 py-2 rounded-md ${bgClass}`}
                        >
                ...
                <div className="flex flex-col gap-2">
                  <div
                    className={`flex items-center gap-3 px-3 py-2 rounded-md ${isCorrect ? "bg-[#d1f4e0]" : "bg-[#fce8e6]"}`}
                  >
                    {isCorrect ? (
                      <CheckCircle
                        size={20}
                    ...
                    <span
                      className={`text-sm ${isCorrect ? "text-[#43C17A]" : "text-[#FF3B30]"}`}
                    >
                      {t("Your answer:")}{" "}
