  ...
  return (
    <>
      ...
      <div
        ...
        className="hidden md:flex bg-white rounded-2xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.06)] border border-gray-100 flex-col gap-4 cursor-pointer hover:shadow-md transition-shadow"
      >
      ...
      <div
        onClick={handleCardClick}
        className="md:hidden bg-[#F4F5F6] rounded-xl p-4 flex flex-col gap-2.5 mb-4 shadow-sm border border-gray-100"
      >
        <div className="flex justify-between items-start">
          <h3 className="text-[#282828] font-bold text-[15px] pr-2">
            {data.title}
          </h3>
          {!isCompleted ? (
            <button
              onClick={handleUploadClick}
              className="bg-[#43C17A] text-white px-4 py-1.5 rounded-md text-[11px] font-bold shrink-0 shadow-sm"
            >
              {t("Upload")}
            </button>
          ) : (
            <div className="bg-[#16284F] text-white px-3 py-1 rounded-md text-[11px] font-bold shrink-0 shadow-sm">
              {t("Uploaded")}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1.5 mt-2 w-full">
          <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
            <UserCircle size={14} className="text-[#43C17A]" />{" "}
            <span>Faculty Name: {data.facultyName}</span>
          </div>
          <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
            <CalendarDotsIcon size={14} className="text-[#43C17A]" />{" "}
            <span>
              Uploaded On:{" "}
              {data.createdAt
                ? new Date(data.createdAt).toLocaleDateString()
        ...
        <div className="mt-3">
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide mt-1.5 pb-1">
            {(data.attachments ?? []).map(
              (file: { fileUrl: string }, idx: number) => (
                <a
                  key={`fac-m-${idx}`}
                  ...
                  rel="noopener noreferrer"
                  onClick={(e) => e.stopPropagation()}
                  className="flex items-center gap-1.5 bg-[#e2e8f0] text-[#334155] px-2.5 py-1 rounded border border-slate-300 text-[10px] font-medium shrink-0"
                >
                  <FilePdf size={14} className="text-[#16284F]" weight="fill" />
            ...
            {uploadedFiles &&
              uploadedFiles.map((file: any, idx: number) => (
                <div
                  key={`stu-m-${idx}`}
                  className="flex items-center gap-1.5 bg-[#e2f6ea] text-[#334155] px-2.5 py-1 rounded border border-[#43C17A]/30 text-[10px] font-medium shrink-0"
                >
                  <a
                    href={file.fileUrl}
                    target="_blank"
