...
import {
  joinClubAPI,
  getStudentClubStatusAPI,
} from "@/lib/helpers/clubActivity/studentClubAPI";
import { useEffect, useState } from "react";
import { useUser } from "@/app/utils/context/UserContext";
import { Avatar } from "@/app/utils/Avatar";
import { Pagination } from "@/app/(screens)/admin/academic-setup/components/pagination";
...
export default function AllClubsGrid() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const clubsPromise = getAllClubsAPI(
        ...
        if (studentId) {
          statusPromise = getStudentClubStatusAPI(
            parseInt(studentId.toString(), 10),
          );
        }
        ...
        toast.error(t("Failed to load clubs or status"), {
          id: "fetch-clubs-error",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleJoinClick = async (clubId: string) => {
    if (!studentId) {
      toast.error(t("Student ID is missing Cannot send request"));
      return;
    }
    setJoiningClubId(clubId);
    try {
      ...
      toast.error(
        error.message || t("Failed to send request Please try again"),
      );
    } finally {
      setJoiningClubId(null);
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-24 pt-16 content-start animate-in fade-in duration-500">
        {clubs.map((club) => {
          ...
          if (isThisClubTheActiveOne && isPendingForAClub) {
            ...
            buttonClass =
              "bg-[#43C17A] text-white hover:bg-[#35a164] active:scale-95 cursor-pointer";
          } else if (isButtonDisabled) {
            buttonClass = "bg-gray-300 text-gray-500 cursor-not-allowed";
          }
