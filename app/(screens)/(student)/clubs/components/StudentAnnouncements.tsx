...
} from "@/lib/helpers/clubActivity/studentAnnouncementAPI";
import { StudentAnnouncementsShimmer } from "../shimmers/StudentAnnouncementsShimmer";
import ConfirmDeleteModal from "@/app/(screens)/admin/calendar/components/ConfirmDeleteModal";
import { Avatar } from "@/app/utils/Avatar";
...
export const getMessageDateHeader = (dateString: string, t: any) => {
  const date = new Date(dateString);
  const today = new Date();
  const yesterday = new Date();
  yesterday.setDate(today.getDate() - 1);
...
export const formatRoleDisplay = (role: string, t: any) => {
  switch (role?.toLowerCase()) {
    case "president":
      return t("President");
    case "vicepresident":
...
export default function StudentAnnouncements({
  ...
  const appendMessage = (msg: any) => {
    setMessages((prev) => {
      if (prev.some((m) => m.announcementId === msg.announcementId))
        return prev;
      return [...prev, msg];
    });
    setTimeout(() => scrollToBottom("smooth"), 100);
  ...
  useEffect(() => {
    ...
    const loadInitial = async () => {
      try {
        ...
        if (!abortController.signal.aborted) {
          ...
          if (data.length > 0 && onDateChangeRef.current) {
            onDateChangeRef.current(
              getMessageDateHeader(data[data.length - 1].createdAt, t),
            );
          }
          setTimeout(() => scrollToBottom("auto"), 100);
        }
        ...
        if (
          ...
          toast.error(t("Network is unstable but we are trying to connect"), {
            id: "load-err",
          });
        }
      } finally {
    ...
    const channel = subscribeToClubAnnouncements(clubId, {
      onInsertBroadcast: (payload) => {
        ...
        appendMessage(payload);
        if (onDateChangeRef.current) onDateChangeRef.current(t("Today"));
      },
      onPostgresFallback: (payload) => {
        if (abortController.signal.aborted) return;
        ...
        setTimeout(() => {
          if (
            abortController.signal.aborted ||
            processedIds.current.has(payload.new.announcementId)
          )
            return;

          fetchSingleAnnouncement(payload.new.announcementId)
            .then((data) => {
              if (data && !abortController.signal.aborted) {
                processedIds.current.add(data.announcementId);
                appendMessage(data);
                if (onDateChangeRef.current)
                  onDateChangeRef.current(t("Today"));
              }
            })
            .catch(() => console.warn("Fallback fetch failed."));
        }, 2000);
      ...
      onUpdate: (updatedRow) => {
        ...
        if (updatedRow.is_deleted) {
          setMessages((prev) =>
            prev.filter((m) => m.announcementId !== updatedRow.announcementId),
          );
        } else {
          setMessages((prev) =>
            prev.map((m) =>
              m.announcementId === updatedRow.announcementId
                ? {
                    ...m,
  ...
  const fetchOlderMessages = async () => {
    ...
    const oldestMessage = messages[0];
    try {
      const olderMessages = await fetchAnnouncements(
        clubId,
      ...
      setMessages((prev) => {
        const existingIds = new Set(prev.map((m) => m.announcementId));
        const newUniqueMessages = olderMessages
          .reverse()
          .filter((m) => !existingIds.has(m.announcementId));
        return [...newUniqueMessages, ...prev];
      });
      ...
      toast.error(t("Failed to load older messages"), { id: "pg-err" });
    } finally {
      isFetchingRef.current = false;
      setFetchingMore(false);
    }
  ...
  const handleScroll = (e: UIEvent<HTMLDivElement>) => {
    ...
    if (onDateChange) {
      ...
      for (let i = 0; i < messageElements.length; i++) {
        const el = messageElements[i] as HTMLElement;
        if (el.offsetTop - target.offsetTop >= target.scrollTop - 20) {
          onDateChange(el.getAttribute("data-date") || t("Today"));
          break;
        }
      }
    }
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    const textToPost = inputValue.trim();
    if (!textToPost || textToPost.length > 1000 || !canPost || isPosting)
      return;
    ...
    try {
      if (editingId) {
        ...
        setEditingId(null);
        toast.success(t("Announcement updated"), { id: "upd-success" });

        setInputValue("");
      } else {
        const newMessage = await postStudentAnnouncement(
          clubId,
          collegeId,
          studentId,
          userRole || "president",
      ...
      toast.error(t("Slow connection Failed to post"), { id: "post-err" });
    } finally {
      setIsPosting(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteAnnouncement(selectedDeleteId);
      setMessages((prev) =>
        prev.filter((m) => m.announcementId !== selectedDeleteId),
      );
      toast.success(t("Announcement deleted"), { id: "del-success" });
      setDeleteModalOpen(false);
    } catch (err) {
      toast.error(t("Failed to delete"), { id: "del-err" });
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const getAuthorDetails = (announcement: any) => {
    if (announcement.authorStudent) {
      const profile = Array.isArray(
        ...
        : announcement.authorStudent.users?.user_profile;
      return {
        name: announcement.authorStudent.users?.fullName,
    ...
    if (announcement.authorFaculty) {
      const profile = Array.isArray(
        ...
        : announcement.authorFaculty.users?.user_profile;
      return {
        name: announcement.authorFaculty.users?.fullName,
        avatar: profile?.profileUrl,
      };
    }
    return { name: "Unknown", avatar: null };
      ...
      >
        {messages.length === 0 && !loading && (
          <div className="flex h-full flex-col items-center justify-center text-center opacity-80 animate-in fade-in duration-500">
            ...
            <p className="text-lg font-bold text-[#16284F] mb-1">
              {t("No Announcements Yet")}
            </p>
            <p className="text-sm font-medium text-gray-500 max-w-xs">
              {canPost
                ? t(
                    "Be the first to share an update or important information with the club!",
                  )
        ...
        <div className="flex flex-col gap-4 pb-2 min-h-full justify-end">
          {hasMore && messages.length > 0 && (
            <div className="flex justify-center py-2 animate-in fade-in">
              <button
                ...
                className="cursor-pointer text-xs font-bold text-gray-400 hover:text-[#16284F] transition-colors py-2 px-4 rounded-full bg-gray-50 hover:bg-gray-100 disabled:opacity-50 border border-gray-200"
              >
                {fetchingMore ? (
                  <span className="h-3 w-3 animate-spin rounded-full border-2 border-[#16284F] border-t-transparent inline-block align-middle mr-2"></span>
                ) : null}
                {fetchingMore ? t("Loading") : t("Load older messages")}
              </button>
            </div>
          )}
          ...
          {messages.map((announcement) => {
            const authorInfo = getAuthorDetails(announcement);
            const isMyMessage = announcement.authorStudentId === studentId;
            const dateStr = getMessageDateHeader(announcement.createdAt, t);
            const formattedRole = formatRoleDisplay(announcement.authorRole, t);
                  ...
                  <div className="flex-1 rounded-2xl bg-white p-4 shadow-md border-2 border-[#CCCCCC] transition-all hover:border-[#16284F]/30 relative">
                        ...
                        <div className="absolute z-50 right-0 top-full mt-0 hidden w-32 flex-col rounded-xl border border-gray-100 bg-white p-1.5 shadow-xl group-hover/menu:flex animate-in fade-in zoom-in-95 duration-200">
                          <button
                            onClick={() => {
                              ...
                              setInputValue(announcement.message);
                            }}
                            className="cursor-pointer flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-50 hover:text-[#16284F]"
                          >
                            <PencilSimple size={16} weight="bold" /> {t("Edit")}
                          </button>
                          <button
                            onClick={() => {
                              setSelectedDeleteId(announcement.announcementId);
                              setDeleteModalOpen(true);
                            }}
                            className="cursor-pointer flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-sm font-medium text-red-500 transition-colors hover:bg-red-50 hover:text-red-600"
                          >
                    ...
                    {announcement.updatedAt &&
                      new Date(announcement.updatedAt).getTime() >
                        new Date(announcement.createdAt).getTime() + 1000 && (
                        <div className="flex justify-end mt-1">
                          <span className="text-[10px] font-bold text-gray-400">
                            {t("Edited")}
                          </span>
