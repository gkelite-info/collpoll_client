...
export function NewVisitorEntryModal({ onClose, onSave }: { onClose: () => void; onSave: () => void }) {
      ...
      <div className="relative max-h-[calc(100vh-1.5rem)] w-full max-w-[720px] overflow-y-auto rounded-lg bg-white shadow-2xl">
        ...
        <div className="space-y-4 px-5 py-4">
          ...
          <section>
            ...
            <div className="grid gap-3 md:grid-cols-2"><label className={labelClass}>Entry Date <span className="text-red-500">*</span><input type="date" defaultValue="2025-05-15" className={inputClass} /></label><label className={labelClass}>Entry Time <span className="text-red-500">*</span><input type="time" defaultValue="10:15" className={inputClass} /></label></div>
          ...
          <section>
            <h3 className="mb-3 flex items-center gap-2 text-sm font-extrabold text-[#334155]"><Wrench size={16} className="text-[#16A85B]" />4. Equipment Details</h3>
            <div className="grid gap-3 md:grid-cols-[1.2fr_.7fr_1fr_1.2fr]"><label className={labelClass}>Equipment <span className="text-red-500">*</span><select className={inputClass} defaultValue=""><option value="" disabled>Select equipment</option><option>Football</option><option>Cricket Bat</option></select></label><label className={labelClass}>Quantity <span className="text-red-500">*</span><input type="number" defaultValue="1" min="1" className={inputClass} /></label><label className={labelClass}>Condition <span className="text-red-500">*</span><select className={inputClass} defaultValue="Good"><option>Good</option><option>Fair</option></select></label><label className={labelClass}>Remarks <span className="font-normal text-[#94A3B8]">(Optional)</span><input className={inputClass} placeholder="Enter remarks" /></label></div>
            <button type="button" className="mt-3 inline-flex h-8 cursor-pointer items-center gap-2 rounded border border-[#16A85B] px-3 text-xs font-bold text-[#149447]"><Plus size={13} weight="bold" />Add Another Equipment</button>
          </section>
        </div>
