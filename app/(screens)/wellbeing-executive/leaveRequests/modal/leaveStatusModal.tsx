...
interface WellbeingLeaveDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  leaveData: any;
}

export default function WellbeingLeaveDetailsModal({
  isOpen,
  onClose,
  leaveData,
}: WellbeingLeaveDetailsModalProps) {
      ...
      setMessages([
        {
          chatId: 1,
          senderRole: "STUDENT",
          senderName: leaveData.name,
          senderAvatar: leaveData.photo,
          message: `Hello sir, please approve my leave. ${leaveData.description}`,
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const savedMsg = {
        ...
        senderRole: "FACULTY",
        message: msgText,
        mediaUrl: fileObj ? URL.createObjectURL(fileObj) : undefined,
        mediaType: fileObj?.type.startsWith("image/") ? "image" : "document",
      ...
      <div className="bg-gray-50 rounded-xl shadow-2xl w-full max-w-[850px] h-[80vh] flex overflow-hidden animate-in fade-in duration-200 relative">
        ...
        <div className="w-[30%] min-w-[220px] bg-gray-50/50 border-r border-gray-200 p-3 flex flex-col gap-2.5 overflow-y-auto custom-scrollbar">
          <div className="bg-[#10B9810F] border border-gray-200 rounded-lg p-3 flex flex-col items-center shadow-sm">
            <Avatar src={leaveData.photo} size={48} alt="Student" />
            <h3 className="font-bold text-[#282828] text-[13px] text-center leading-tight truncate w-full mt-2">
              {leaveData.name}
            </h3>
                ...
                {messages.length === 0 && !isSending ? (
                  ...
                  messages.map((msg, idx) => {
                    ...
                    return (
                      <div
                        key={`${msg.chatId}-${idx}`}
                        className={`flex gap-1.5 w-full max-w-[85%] ${
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            className={`px-2.5 py-2 rounded-xl text-[12px] relative ${
                              isMe
