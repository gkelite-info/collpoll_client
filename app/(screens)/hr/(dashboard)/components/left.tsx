...
import { MyAttendanceLeft } from "../../MyAttendance/left";
...
const formatRoleForUrl = (roleStr: string) => {
  return roleStr
    .split(" ")
    .map((word, i) =>
      i === 0
  ...
  return (
    <div className="flex gap-3 w-full">
      {[...Array(4)].map((_, i) => (
        <div
          key={i}
          className="flex-1 h-[126px] rounded-xl bg-gray-200 relative overflow-hidden"
        >
          <div className="absolute inset-0 -translate-x-full animate-[shimmer_1.4s_infinite] bg-gradient-to-r from-transparent via-white/60 to-transparent" />
        </div>
      ))}
...
export default function HrDashLeft() {
  ...
  const handleViewClick = (userId: number) => {
    ...
    const formattedUrlRole = formatRoleForUrl(activeRole);

    currentUrl.searchParams.set("role", formattedUrlRole);
  ...
  const fetchDetail = async (month: string, page: number) => {
    ...
    const res = await getMonthDetail(
      collegeId,
      activeRole,
      currentYear,
      idx,
  ...
  const roleLabel =
    HR_ROLE_PILLS.find((p) => p.value === activeRole)?.label ?? activeRole;
  ...
  if (urlUserId) {
    return (
      <div ref={topRef} className="w-[68%] p-2">
        <MyAttendanceLeft userId={urlUserId} onBack={handleBackFromStaffView} />
      </div>
    );
  }
            ...
            {HR_ROLE_PILLS.map((pill) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors cursor-pointer ${isActive ? "bg-[#22C55E] text-white border-[#22C55E]" : "bg-white text-[#282828] border-gray-300 hover:bg-[#22C55E] hover:text-white hover:border-[#22C55E]"}`}
                >
                  {pill.label}
                </button>
              );
