...
export default function HrStaffSummaryPage({ userId, profile }: { userId: string; profile?: FacultyProfileData | null }) {
  return (
    <div className="w-full grid grid-cols-1 xl:grid-cols-2 gap-4 text-left">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-[0px_4px_20px_rgba(0,0,0,0.03)] border border-gray-50">
        <div className="flex flex-col items-center mb-6 mt-2">
          <img
            src={profile?.image || "/rahul.png"}
            alt={profile?.name || "Harsha Sharma"}
            className="w-[84px] h-[84px] rounded-full object-cover mb-3 shadow-sm"
          />
          <h2 className="text-[17px] font-bold text-gray-800 text-center">{profile?.name || "Harsha Sharma"}</h2>
        </div>
        <div className="flex flex-col space-y-0.5">
          <InfoRow label="ID" value={profile?.employeeId || "9046928764"} />
          <InfoRow label="Department" value={profile?.department || "CSE"} />
          <InfoRow label="Mobile" value={profile?.mobile || "9876432134"} />
          <InfoRow label="Email" value={profile?.email || "sai@gmail.com"} />
          <InfoRow label="Date of Joining" value={profile?.joiningDate || "12 July 2019"} />
          <InfoRow label="Experience" value={profile?.experience || "6 years"} />
        </div>
      </div>
