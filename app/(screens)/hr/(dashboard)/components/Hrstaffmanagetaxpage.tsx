...
type TaxTab = "declaration" | "forms" | "taxFiling" | "taxSaving";

import { FacultyProfileData } from "@/lib/helpers/Hr/myAttendance/fetchFaculty";

export default function HrStaffManageTaxPage({ userId, profile }: { userId: string; profile?: FacultyProfileData | null }) {
  const [activeTab, setActiveTab] = useState<TaxTab>("declaration");
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col h-[600px] text-left">
        ...
        {tabs.map((tab, index) => (
          <React.Fragment key={tab.id}>
            <span
              ...
              className={`cursor-pointer transition-colors ${activeTab === tab.id ? "text-[#43C17A] underline decoration-2 underline-offset-4" : "text-[#333333] hover:text-[#43C17A]"}`}
            >
              {tab.label}
            </span>
            {index < tabs.length - 1 && <span className="text-gray-400 mx-2">/</span>}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white rounded-md p-6 shadow-[0px_2px_10px_rgba(0,0,0,0.04)] border border-gray-100">
              ...
              <p className="text-[#1F2937] text-base leading-relaxed">
                Form 12BB is a declaration of investments and expenses submitted to your employer for accurate TDS deduction.
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === "taxFiling" && (
          <div>
            <h3 className="text-[#43C17A] font-bold text-[20px] mb-2">Tax Filing</h3>
            <p className="text-[#1F2937] text-base mb-4">
            ...
            <div className="border border-gray-400 rounded-md px-4 py-3 mb-6 w-fit bg-white">
              <p className="text-black text-sm border-[#5E5E5E]">
            ...
            <div className="border border-gray-300 rounded-md p-6 bg-white">
              <h4 className="text-[#1F2937] font-semibold text-lg mb-2">GK's Choice</h4>
              <p className="text-[#1F2937] text-base leading-relaxed mb-6">
                SmartTax is a government-authorized e-filing platform integrated with HRMS to simplify tax filing for employees.
                It enables auto-fetching of Form 16, deductions, and investment details directly from your HRMS portal — making filing accurate and hassle-free.
              </p>
              <div className="mb-6">
                <h5 className="text-[#1F2937] font-semibold text-base mb-3">Why Choose GK :</h5>
                <ul className="space-y-2">
                  {[
                    ...
                    <li key={i} className="flex items-center gap-2 text-base text-[#1F2937]">
                      <CheckCircle size={18} weight="fill" className="text-[#43C17A]" />
                      {item}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                <h5 className="text-[#1F2937] font-semibold text-base mb-3">How to File Via Smart Tax</h5>
        ...
        {activeTab === "taxSaving" && (
          <div className="bg-white rounded-lg p-8 border border-gray-200 shadow-sm flex flex-col gap-6">
            <div>
              <h3 className="text-[#43C17A] font-bold text-[20px] mb-2">Tax Saving Investment</h3>
            ...
            <div className="border border-black rounded-md p-6 bg-white">
              <h4 className="text-[#1F2937] font-semibold text-base mb-4">Grow Your Financial Wellness</h4>
              <ul className="space-y-3 text-base text-[#1F2937]">
                {[
