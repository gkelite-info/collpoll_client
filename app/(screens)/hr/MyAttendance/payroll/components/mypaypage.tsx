...
export default function MyPayPage({ profile }: MyPayPageProps) {
  ...
  const loadPayData = async () => {
    if (!actualUserId || !collegeId) return;
    setIsFetchingPay(true);
    try {
      const data = await fetchEmployeePaySummary(
        parseInt(actualUserId as string, 10),
        collegeId,
      );
      setPayData(data);
    } catch (error) {
  ...
  }, [actualUserId, collegeId, hrLoading]);
  ...
  const employeeModalData = {
    ...profile,
    userId: parseInt(actualUserId as string, 10),
        ...
        <>
          <div className="flex-shrink-0">
            <div className="flex justify-between items-center mb-3">
              ...
              >
                {totalCTC > 0 ? "Edit Pay" : "Adding Pay"}
              </button>
            </div>
