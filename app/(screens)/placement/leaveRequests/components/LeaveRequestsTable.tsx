...
import { type PlacementLeaveRequest } from "../data";
...
const mapDbRequestToPlacementRow = (
  request: EmployeeLeaveRequestRecord,
  serialNumber: number,
): PlacementLeaveRequest => {
  const employeeCode = request.employee?.employeeId ?? String(request.employeeId);
  const requesterName = request.user?.fullName ?? "Placement Officer";
...
export default function LeaveRequestsTable({ view }: LeaveRequestsTableProps) {
  ...
  const [requests, setRequests] = useState<PlacementLeaveRequest[]>([]);
  const [totalCount, setTotalCount] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedRequest, setSelectedRequest] =
    useState<PlacementLeaveRequest | null>(null);
        ...
        data.map((request, index) => ({
          ...
          name:
            view === "tagged"
              ...
              : request.user?.fullName ?? fullName ?? "Placement Officer",
        })),
      );
