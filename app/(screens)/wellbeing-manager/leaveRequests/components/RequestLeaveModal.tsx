...
export default function RequestLeaveModal({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!userId || !collegeId || !role || userLoading) {
      toast.error("Wellbeing manager session not found. Please re-login.");
      return;
    }

    if (role !== "WellbeingManager") {
      toast.error("This request is available for wellbeing managers only.");
      return;
    }
