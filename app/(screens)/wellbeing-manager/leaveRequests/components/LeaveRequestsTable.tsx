...
import { type WellbeingManagerLeaveRequest } from "../data";
...
const mapDbRequestToWellbeingManagerRow = (
  request: EmployeeLeaveRequestRecord,
  serialNumber: number,
): WellbeingManagerLeaveRequest => {
  const employeeCode = request.employee?.employeeId ?? String(request.employeeId);
  const requesterName = request.user?.fullName ?? "Wellbeing Manager";
...
export default function LeaveRequestsTable({ view }: LeaveRequestsTableProps) {
  ...
  const [requests, setRequests] = useState<WellbeingManagerLeaveRequest[]>([]);
  const [totalCount, setTotalCount] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedRequest, setSelectedRequest] =
    useState<WellbeingManagerLeaveRequest | null>(null);
    ...
    try {
      ...
      setRequests(
        data.map((request, index) => ({
          ...mapDbRequestToWellbeingManagerRow(
            request,
          ...
          name:
            view === "tagged"
              ...
              : request.user?.fullName ?? fullName ?? "Wellbeing Manager",
        })),
      );
