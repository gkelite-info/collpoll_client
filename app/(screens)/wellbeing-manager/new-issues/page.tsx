...
// MARK: - FILE: page.tsx (CORE SUITE SYSTEM CONTROLLER)
"use client";

import { Suspense } from "react";
import { useSearchParams } from "next/navigation";
import { Loader } from "../../(student)/calendar/right/timetable";
import ReassignedIssuesView from "./components/ReassignedIssuesView";
import NewIssuesPageContent from "./components/NewIssuesView";


function DashboardRouterView() {
  const searchParams = useSearchParams();
  const currentTab = searchParams.get("tab") || "new";

  if (currentTab === "reassigned") {
    return <ReassignedIssuesView />;
  }

  return <NewIssuesPageContent />;
}
...
export default function NewIssuesPage() {
  return (
    <Suspense 
      fallback={
        <div className="flex py-10 justify-center bg-slate-50 text-xs font-bold tracking-wide text-[#16284F]/50">
          <Loader />
        </div>
      }
    >
