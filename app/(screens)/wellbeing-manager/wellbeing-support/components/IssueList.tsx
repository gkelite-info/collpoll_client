...
import { mockIssues, wellbeingCategories } from "../data";
...
export default function IssueList() {
  ...
  }, [currentTab, currentPage, selectedCategory]);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      const target = event.target as HTMLElement;
      if (!target.closest(".custom-category-dropdown-container")) {
  ...
  useEffect(() => {
    ...
    if (params.get("page") !== "1" && params.get("page") !== null) {
      params.set("page", "1");
  ...
  const getIssues = () => {
    ...
    switch (currentTab) {
      case "pending":
        issues = mockIssues.pending;
        break;
      case "resolved":
        issues = mockIssues.resolved;
        break;
      case "rejected":
        issues = mockIssues.rejected;
        break;
      case "raised":
      default:
        issues = mockIssues.raised;
        break;
    }
    ...
    if (selectedCategory.toLowerCase() !== "all") {
      issues = issues.filter(
        (issue) =>
          issue.subCategory.toLowerCase() === selectedCategory.toLowerCase()
      );
    }
    ...
    <div className="flex-1 flex flex-col mt-6 w-full max-w-3xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-4 px-2">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-start sm:justify-end">
          <div className="relative custom-category-dropdown-container">
            <button
              onClick={() => setIsCategoryDropdownOpen(!isCategoryDropdownOpen)}
              className="flex cursor-pointer items-center justify-between gap-1.5 bg-[#43C17A] hover:bg-[#36a666] text-white px-3 sm:px-4 py-2 rounded-lg text-sm sm:text-base font-medium transition-all shadow-sm focus:outline-none min-w-[120px]"
            >
            ...
            {isCategoryDropdownOpen && (
              <div className="absolute left-0 mt-1 w-[180px] bg-white border border-gray-100 rounded-lg shadow-lg py-1 z-20 overflow-hidden">
                {["all", ...wellbeingCategories].map((cat) => (
                  <button
                    key={cat}
                    onClick={() => {
          ...
          <div className="relative custom-dropdown-container">
            ...
            {isDropdownOpen && (
              <div className="absolute left-0 sm:right-0 mt-1 w-full min-w-[120px] bg-white border border-gray-100 rounded-lg shadow-lg py-1 z-20 overflow-hidden">
