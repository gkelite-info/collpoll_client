...
import { fetchTaggedEmployeeLeaveRequestCounts } from "@/lib/helpers/employeeLeaveRequests/employeeLeaveRequestAPI";
...
export default function LeaveSummaryCards() {
  ...
  const { collegeId, loading: collegeAdminLoading } = useCollegeAdmin();
  ...
  const loadCounts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching college admin leave summary counts:", error);
      setCounts({ total: 0, approved: 0, pending: 0, rejected: 0 });
    } finally {
      setIsLoading(false);
    }
  }, [collegeAdminLoading, collegeId, userId, userLoading]);
  ...
  useEffect(() => {
    const handleUpdated = () => loadCounts();
    window.addEventListener("employee-leave-request-created", handleUpdated);
    return () =>
      window.removeEventListener("employee-leave-request-created", handleUpdated);
  }, [loadCounts]);
      ...
      {cards.map((card) => {
        ...
        return isLoading ? (
          ...
          <CardComponent
            ...
            style={`w-full transition-all duration-300 ${
