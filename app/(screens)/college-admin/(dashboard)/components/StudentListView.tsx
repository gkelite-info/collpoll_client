...
import { CaretLeft, CaretDown, CaretRight, MagnifyingGlass,
...
// ── ADDED: TableShimmer (same as AdminListView / FacultyListView) ──────────────
...
export default function StudentListView({ onBack }: Props) {
  ...
  const [summary, setSummary]       = useState<StudentPageSummary | null>(null); // ← ADDED
  ...
  // ── ADDED: Query routing — sets ?subview=students, cleans up on unmount ──
  useEffect(() => {
    const params = new URLSearchParams(searchParams.toString());
    params.set("subview", "students");
  ...
  const load = useCallback(async (page: number, eduId: number | null, branch: string, year: string, admin: string, searchTerm: string) => { // ← added searchTerm
  ...
  useEffect(() => {
    if (contextLoading || !collegeId) return;
    load(1, null, "All", "All", "All", ""); // ← added ""
  }, [collegeId, contextLoading]);

  // ── ADDED: debouncedSearch included in dependencies ──
  useEffect(() => {
    if (!collegeId || contextLoading) return;
    load(currentPage, selectedEduId, selectedBranch, selectedYear, selectedAdmin, debouncedSearch);
  }, [currentPage, selectedEduId, selectedBranch, selectedYear, selectedAdmin, debouncedSearch]);
  ...
  const tableData = useMemo(() => {
    if (!data) return [];
    return data.students; // ← removed client-side search filter; API does it
  }, [data]);
