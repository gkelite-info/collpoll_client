...
export default function ParentListView({ onBack }: Props) {
  ...
  const load = async (page: number, eduId: number | null, branch: string, year: string) => {
    if (contextLoading || !collegeId) return;
    setIsFetching(true);
    try {
      // Resolve IDs from names for server-side filtering
  ...
  useEffect(() => {
    ...
    load(currentPage, selectedEduId, selectedBranch, selectedYear);
  }, [currentPage, selectedEduId, selectedBranch, selectedYear]);
  ...
  const tableData = useMemo(() => {
    ...
    return data.parents.filter((p) => {
      const matchSearch = !search ||
        ...
        p.linkedStudent.toLowerCase().includes(search.toLowerCase()) ||
        String(p.parentId).includes(search);
      return matchSearch;
    });
  }, [data, search]);
