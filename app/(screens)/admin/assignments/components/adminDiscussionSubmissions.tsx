...
const MOCK_SUBMISSIONS = [
    ...
    { id: 4, name: "Reema Sajid", studentId: "479210", section: "A", submittedOn: "04/09/2026", file: "AI_Education_Project.pdf", marksObtained: 20, totalMarks: 25, avatar: "https://i.pravatar.cc/100?u=5" },
];

export default function AdminDiscussionSubmissions({ discussionId }: { discussionId: string | null }) {
    ...
    return (
        <div className="flex flex-col mx-auto h-full pb-10">
            <div className="flex items-center gap-2 cursor-pointer mb-5 text-[#282828] hover:text-black transition-colors">
                <CaretLeft size={24} weight="bold" onClick={handleBack}/>
            ...
            <div className="flex flex-col gap-4 overflow-y-auto max-h-[70vh] scrollbar-hide">
                {MOCK_SUBMISSIONS.map((student) => (
                    <div key={student.id} className="bg-white overflow-x-auto rounded-xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-gray-100 flex gap-3">
                        <div className="flex-shrink-0 items-center">
                            <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden relative">
                                <img src={student.avatar} alt={student.name} className="object-cover w-full h-full" />
                            </div>
                        </div>
