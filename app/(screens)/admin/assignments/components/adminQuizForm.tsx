...
export default function AdminQuizForm({ onCancel }: AdminQuizFormProps) {
  ...
  // FIX: Using array index guarantees a unique selection and prevents NaN errors
  ...
  const handleSave = async (status: "Draft" | "Active") => {
    ...
    // Safely retrieve the exact Unit ID using the array index
    const selectedTopicObj = topics[selectedTopicIndex];
    ...
    try {
      ...
      const result = await saveQuiz({
        ...
        collegeSubjectId: Number(subjectId),
        collegeSectionsId: sectionId,
        collegeSubjectUnitId: selectedTopicObj.collegeSubjectUnitId, // Safely mapped back
        quizTitle: quizTitle.trim(),
        totalMarks: Number(totalMarks),
        startDate,
        endDate,
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              <option value="">Select Topic</option>
              {topics.map((topic, index) => (
                // Use the map index as a 100% reliable, unique value
