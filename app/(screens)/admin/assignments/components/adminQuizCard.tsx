    ...
    <div className="bg-white rounded-xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.06)] border border-gray-100 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2.5 mt-2">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          {quizView === "drafts" ? (
            <button
              onClick={() => onPublish?.(data.quizId || data.id)}
              className="bg-[#16284F] text-white px-6 py-2 rounded-md font-bold text-sm hover:bg-[#102040] transition-colors cursor-pointer"
            >
              Publish
            </button>
          ) : (
