...
import { updateStudentDiscussionMarks } from "@/lib/helpers/student/assignments/discussionForum/student_discussion_uploadsAPI";
...
interface AddMarksModalProps {
  ...
  onClose: () => void;
  student: any;
  discussionId: number; // 🟢 Explicitly added to guarantee database updates work
  onSuccess: () => void;
}

export default function AdminAddMarksModal({
  isOpen,
  onClose,
  student,
  discussionId,
  ...
  const handleSave = async () => {
    ...
    if (numMarks > (student.totalMarks || 25)) {
      toast.error(
        `Marks cannot exceed total marks (${student.totalMarks || 25})`,
      );
      return;
    }
    ...
    setIsSaving(true);
    try {
      // 🟢 Pass the explicit discussionId so the DB query finds the row!
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-[400px] p-6 shadow-xl relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <h3 className="text-[#43C17A] font-bold text-sm mb-4">
          {student.profiles?.full_name || student.name || "Unknown Student"}
        </h3>
        ...
        <div className="flex justify-center items-center gap-3 mb-8">
          <input
            ...
            onChange={(e) => {
              ...
              const maxMarks = student.totalMarks || 25;
              if (val !== "") {
