...
import { CardProps } from "../../components/subjectCards";
...
function FilterBanner(filterBannerDetails: FilterBannerProps) {
    ...
    const rawSubject = params.id;
    const router = useRouter();

    const formatSlugToTitle = (value: string) => {
        return value
            .replace(/[-_]+/g, " ")
            .replace(/[()]/g, "")         
            .replace(/\s+/g, " ")
    ...
    const subjectTitle =
        typeof rawSubject === "string"
            ? formatSlugToTitle(decodeURIComponent(rawSubject))
            : "";
