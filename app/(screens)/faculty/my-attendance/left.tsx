...
const MyAttendanceLeft = () => {
  ...
  const urlMainTab = (searchParams.get("main") as "attendance" | "payroll" | "analytics") || "payroll";
  const urlSubTab = (searchParams.get("sub") as "summary" | "myPay" | "manageTax") || "summary";
 
  const [activeMainTab, setActiveMainTab] = useState<"attendance" | "payroll" | "analytics">(urlMainTab);
  const [activePayrollTab, setActivePayrollTab] = useState<"summary" | "myPay" | "manageTax">(urlSubTab);
  ...
  const handleMainTabClick = (id: string) => {
    console.log("👆 Main Tab Clicked:", id);
    if (id === activeMainTab) {
      console.log("⚠️ Already on this main tab, skipping router push.");
      return;
    }
   
    setActiveMainTab(id as any);
    const targetPath = id === "payroll" ? `?main=${id}&sub=${activePayrollTab}` : `?main=${id}`;
    console.log("➡️ Routing to:", targetPath);
    router.push(targetPath, { scroll: false });
  };
 
  const handleSubTabClick = (id: string) => {
    console.log("👆 Sub Tab Clicked:", id);
    if (id === activePayrollTab) {
      console.log("⚠️ Already on this sub tab, skipping router push.");
      return;
    }
   
    setActivePayrollTab(id as any);
    console.log("➡️ Routing to sub-path:", `?main=payroll&sub=${id}`);
    router.push(`?main=payroll&sub=${id}`, { scroll: false });
  };
 
  useEffect(() => {
    console.log("🔄 Syncing Left.tsx State with URL:", { urlMainTab, urlSubTab });
   
    if (urlMainTab !== activeMainTab) {
      console.log("✅ Updating Main State:", activeMainTab, "->", urlMainTab);
      setActiveMainTab(urlMainTab);
    }
    if (urlSubTab !== activePayrollTab) {
      console.log("✅ Updating Sub State:", activePayrollTab, "->", urlSubTab);
      setActivePayrollTab(urlSubTab);
    }
  }, [urlMainTab, urlSubTab, activeMainTab, activePayrollTab]);
      ...
      <div className="transition-opacity duration-300 mt-4">
        ...
        {activeMainTab === "payroll" && (
          <div className="flex flex-col items-center w-full p-2">
            ...
            <div className="w-full">
              {activePayrollTab === "summary" && <div className="w-full text-left mt-2"><SummaryPage /></div>}
              {activePayrollTab === "myPay" && <div className="w-full text-left mt-2"><MyPayPage /></div>}
              {activePayrollTab === "manageTax" && <div className="w-full text-left mt-2"><ManageTaxPage /></div>}
            </div>
          </div>
        )}
