...
export default function StudentSettings() {
    ...
    const goTo = (param: string) => {
        ...
        router.push(`/settings?${param}`, { scroll: false });
    };
        ...
        <div className="p-6 space-y-6">
            ...
            {!query &&
                <div className="bg-white shadow-md rounded-xl p-4 space-y-6">
                    <div className="flex items-center justify-between cursor-pointer" onClick={() => goTo("change-password")}>
            ...
            <div className="mt-6">
                {query === "change-password" && <ChangePassword />}
                {/* {query === "email-alerts" && <EmailAlerts />}
                    {query === "language" && <LanguagePreferences />}
                    {query === "linked-accounts" && <LinkedAccounts />}
                    {query === "two-step" && <TwoStep />}
                    {query === "privacy-policy" && <PrivacyPolicy />} */}
            </div>
        </div>
    );
}
