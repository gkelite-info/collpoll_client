...
export default function CreateFee() {
  ...
  const handleSaveModalComponent = () => {
    if (!addDept.trim() || !addCourseType.trim() || !addAmount.trim()) {
      toast.error("Please fill all fields for the fee component.");
      return;
    }
        ...
        if (comps) {
          ...
          comps.forEach((c) => {
            ...
            if (name === "Tuition Fee") newFeeValues["TUITION"] = amount;
              ...
              const gstAmount = Number(amount);
              const subTotal = comps.reduce(
                (acc: number, curr: any) =>
                  curr.fee_type_master?.feeTypeName !== "GST"
                    ? acc + Number(curr.amount)
                    : acc,
                0,
              );
              if (subTotal > 0) {
                setGstValue(`${Math.round((gstAmount / subTotal) * 100)}`);
              } else {
                setGstValue("0");
              }
              gstCalculated = true;
  ...
  const handleSaveFeeStructure = async () => {
    ...
    if (!sessionStart || !sessionEnd)
      return toast.error("Please enter a valid Academic Session (From & To).");
    ...
    if (!metaData.dueDate) return toast.error("Due Date is mandatory.");
    if (!metaData.lateFee || metaData.lateFee.trim() === "")
      return toast.error("Late Fee rule is mandatory. Enter 0 if none.");

    if (!financeManagerId || !collegeId || !collegeEducationId) {
      return toast.error(
        "Session data incomplete. Please refresh and try again.",
      );
    }
    ...
    try {
      ...
      if (componentsToSave.length === 0) {
        toast.error("Please enter an amount for at least one fee component.");
        setIsSaving(false);
        return;
      }
      ...
      const componentPromises = componentsToSave.map(async (comp) => {
        const { success: typeSuccess, feeTypeId } = await saveFeeType(
          comp.label,
        );
        if (typeSuccess && feeTypeId) {
          return saveFeeComponent({
  ...
  const handleSaveAdditionalDues = async () => {
    ...
    if (additionalDuesList.length === 0)
      return toast.error("Please add at least one fee component.");
    if (!additionalMetaData.dueDate)
      return toast.error("Due Date is mandatory.");
    if (!additionalMetaData.lateFee || additionalMetaData.lateFee.trim() === "")
      return toast.error("Late Fee rule is mandatory. Enter 0 if none.");
    ...
    try {
      ...
      const componentPromises = additionalDuesList.map(async (due) => {
        ...
        if (!compResult.success) {
          throw new Error(`Failed to save ${due.dept} component`);
        }
      });
      await Promise.all(componentPromises);
          ...
          >
            <div className="flex flex-col w-full mt-4">
              ...
              <div className="bg-red-00 flex flex-wrap justify-between mt-3 gap-4">
                ...
                <div className="flex flex-col w-[49%]">
                  <div className="flex items-center justify-between">
                    <label className="text-[#282828] font-medium flex items-center gap-1">
            ...
            <div className="bg-pink-00 w-full mt-4 flex flex-col items-start">
              ...
              <div className="bg-yellow-00 w-full flex flex-wrap justify-between mt-3">
                ...
                <div className="flex flex-col w-[49%]">
                  <div className="flex items-center justify-between">
                    <label className="text-[#282828] font-medium">
                      Late Fee Rule <span className="text-red-500">*</span>
                    </label>
                  </div>
                  <input
                    type="text"
