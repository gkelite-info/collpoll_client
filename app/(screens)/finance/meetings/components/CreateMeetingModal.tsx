...
// } from "@/lib/helpers/finance/meetings/meetingsSectionsAPI";
// import {
//   checkFinanceMeetingConflict,
//   fetchFinanceMeetingById,
//   saveFinanceMeeting,
...
//       // 2. HANDLE SECTIONS IF NOT ADMIN (MUST HAPPEN BEFORE SYNC!)
...
//         await scheduleFinanceMeetingReminder(
//           meetingRes.financeMeetingId,
//           date,
//           dbFromTime,
//           reminderMinutes,
...
//                         <option
//                           key={item.collegeBranchId || `branch-${index}`}
//                           value={item.collegeBranchId}
//                         >
...
//                         <option
//                           key={item.collegeAcademicYearId || `year-${index}`}
//                           value={item.collegeAcademicYearId}
//                         >
...
//                     >
//                       <span
//                         className={`truncate ${sectionsSelected.length ? "text-gray-900" : "text-gray-500"}`}
//                       >
...
//                             const checked = sectionsSelected.includes(
...
//                                   checked={checked}
//                                   onChange={() =>
//                                     handleSectionToggle(item.collegeSectionsId)
//                                   }
...
//                   className="w-full border font-medium border-[#E0E0E0] rounded-md px-3 h-[42px] text-sm outline-none bg-white text-[#282828] cursor-pointer"
//                 >
...
const validateTimeRange = (
  ...
  // 🟢 Enforce 4 hour maximum duration
...
const CreateMeetingModal = ({
  ...
  // 🟢 Handles modal open/close cleanup and Smart Time defaults
  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = "hidden";
            ...
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <span
                    className={`text-xs block mb-1 font-semibold transition-colors ${isPastTimeError ? "font-bold text-red-600" : "text-gray-500"}`}
                  >
                    From <span className="text-red-500">*</span>
                  </span>
                  <div className="flex gap-2">
                    <select
                      ...
                      className={`flex-1 px-2 py-2 border rounded-md text-sm bg-white cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#43C17A] transition-colors ${isPastTimeError ? "border-red-500 text-red-600 font-medium" : "border-[#CCCCCC] text-gray-700"}`}
                    >
                      {Array.from({ length: 12 }, (_, i) => {
                        const h = String(i + 1).padStart(2, "0");
                        return (
