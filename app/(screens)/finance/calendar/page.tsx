...
export default function FinanceCalendarPage() {
      ...
      try {
        ...
        const formattedEvents = await Promise.all(
          fetchedEvents.map(async (ev) => {
            ...
            if (sections.length > 0) {
              // Get Branch and Year from the first section (since they are the same for the whole event)
              const firstSec = sections[0] as any;
              ...
              // 🟢 THE FIX: Loop through ALL sections and join their names with a comma
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // Fire toast and update button text FIRST
      toast.success("Event removed successfully");
      setDeleteSuccess(true);

      // Wait 3 full seconds before refreshing
      setTimeout(() => {
        setEventToDelete(null);
        setIsDeleting(false);
