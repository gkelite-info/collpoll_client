...
export default function YearWiseFeeCollection() {
  ...
  const processedData = useMemo(() => {
    ...
    return data.map((item) => {
      ...
      if (item.pendingAmount > 0 && item.paidAmount > 0)
        status = "partial";
      ...
      return {
        ...
        action: (
          <span className="text-[#22A55D] cursor-pointer hover:underline"
          onClick={()=> router.replace(`/finance/${item.rollNo}`)}
          >
            View Details
          </span>
        ),
  ...
  return (
    <div className="p-6 space-y-6 overflow-x-hidden">
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-lg text-[#282828]">Students Overview</h3>
