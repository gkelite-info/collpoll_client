...
import {
  ACCOUNTANT_LEAVE_REQUEST_DB_ROLE,
  type AccountantLeaveRequest,
} from "../data";
...
const mapDbRequestToAccountantRow = (
  request: EmployeeLeaveRequestRecord,
  serialNumber: number,
): AccountantLeaveRequest => {
  const employeeCode = request.employee?.employeeId ?? String(request.employeeId);
  const requesterName = request.user?.fullName ?? "Accountant";
  const requesterRole =
    request.employee?.employeeType === "Accountant"
      ? "Accountant"
...
export default function LeaveRequestsTable({ view }: LeaveRequestsTableProps) {
  const { userId, collegeId, fullName, loading: userLoading } = useUser();
  const [query, setQuery] = useState("");
  const [debouncedQuery, setDebouncedQuery] = useState("");
  const [requests, setRequests] = useState<AccountantLeaveRequest[]>([]);
  const [totalCount, setTotalCount] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedRequest, setSelectedRequest] =
    useState<AccountantLeaveRequest | null>(null);
        ...
        data.map((request, index) => ({
          ...
          name:
            view === "tagged"
              ...
              : request.user?.fullName ?? fullName ?? "Accountant",
        })),
      );
