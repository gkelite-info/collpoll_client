...
export default function ResetPasswordPage() {
  ...
  // 🟢 1. Ref to prevent React Strict Mode from double-firing the code exchange
  const hasExchanged = useRef(false);

  // 🟢 2. Safely exchange the code for a session only ONCE
  useEffect(() => {
    const exchangeCode = async () => {
      ...
      if (code) {
        ...
        if (error) {
          ...
          // Remove the code from the URL bar so it looks clean and doesn't re-trigger
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      // 🟢 3. Directly update the user's password using the session we just grabbed
      ...
      // 🟢 4. Sign the user out so they can cleanly log in with the new password
        ...
        <div className="w-[560px] bg-white rounded-[8px] shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-8">
          ...
          <div className="w-full flex justify-center mt-8">
            <button
              onClick={handleUpdatePassword}
              disabled={loading}
              className="w-[200px] h-[50px] cursor-pointer bg-[#16284F] text-white rounded text-[15px] font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? "Updating..." : "Confirm Password"}
            </button>
          </div>
